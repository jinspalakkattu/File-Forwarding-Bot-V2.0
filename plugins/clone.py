...
async def clone_medias(client: Bot, message: Message):
    ...
    #
    delay = limit = doc_files = video_files = audio_files = voice_files = photo_files = other_files = total_copied = int()
    #
        ...
        for file_type in file_types:
            ...
            if media is not None:
                ...
                if bool(fn_caption) == bool(1):
                    try:
                        caption = str(file_name).rsplit('.', 1)[0] + "\n\n<code>┈••✿ @UniversalFilmStudio ✿••┈</code>"     # ☫File Uploaded By ☞ ⇛@UniversalFilmStudio⇚
                    except Exception:
                        file_name = None
                    ...
                    caption = ""
                total_copied = doc_files + video_files + audio_files + voice_files + photo_files + other_files
                ...
                try:
                    ...
                    # await client.USER.send_document(
                    #     chat_id=target_chat,
                    #     document=source_chat,
                    #     caption=caption if bool(caption) == bool(1) else "",
